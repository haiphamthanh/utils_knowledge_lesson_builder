...
| `templates/<name>/`           | Format, độ sâu mục lục, kiểu đánh số và asset Pandoc | Có                               |
...
| Trung bình | `template.yml` đã điều khiển numbering/toc và dùng chung asset, nhưng các option Pandoc khác vẫn nằm trong `builder.py` | Format cần option mới vẫn có thể phải sửa Python | Chỉ mở rộng contract khi xuất hiện format thực tế; chưa cần plugin/adapter tổng quát |
